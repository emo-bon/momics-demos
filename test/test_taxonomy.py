...
import os
import pandas as pd
from wfs_extra.scripts.mgnify.tables.tables import TaxonomyTable
from wfs_extra.scripts.mgnify.tables.sources.validators import validate_abundance_ncbi
from wfs_extra.scripts.mgnify.tables.sources.detectors import is_emobon_processed
import pytest
...
def test_abundance_and_back(emobon_taxonomy_raw_df):
    ...
    assert validate_abundance_ncbi(abundance_df.df), "Converted DataFrame should be detected as emobon_processed format."

    # Convert back to TaxonomyTable
    back_taxonomy_table = abundance_df.to_taxonomy_table()
    assert type(back_taxonomy_table[2]).__name__== "TaxonomyTable", "Back conversion should yield a TaxonomyTable instance."
    assert back_taxonomy_table[2].df.equals(taxonomy_table.df), "DataFrames should be equal after round-trip conversion."